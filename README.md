...
| **底盘关节电机 ×4** | Unitree 6010 (串口控制, 串联腿关节) |
| **云台电机 ×2** | Pitch (FDCAN2) + Yaw (FDCAN3) |
| **摩擦轮电机 ×6** | DJI M2006 / 双级摩擦轮 (FDCAN3) |
| **拨弹电机 ×1** | DJI M2006 (FDCAN3) |
| **姿态传感器** | Bosch BMI088 (SPI4) + 外部 IMU (FDCAN3) |
...
```
遥控器 (SBUS) ──→ Robo_Task ──→ Chassis_Task ──→ 轮电机 (M3508) + 关节电机 (Unitree)
                      │
...
| 总线 | 外设 | 用途 |
|------|------|------|
| **FDCAN1** | 标准 CAN | 底盘关节电机 (Unitree) + 超级电容 |
...
|------|-----|------|
| 轮半径 | 0.06 m | 底盘驱动轮 |
| 最大 Vx | 4.0 m/s | 前后平移最大速度 |
| 最大 Vy | 4.0 m/s | 左右平移最大速度 |
